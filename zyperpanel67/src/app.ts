...
// Extended server type with allocations
interface ServerWithAllocations {
  id: string;
  name: string;
  type: string;
  version: string;
...
async function getAvailablePorts(allocationId: string): Promise<number[]> {
...
async function assignPortToServer(
  serverId: string,
  allocationId: string,
...
app.get(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    if (primaryAllocation) {
      primaryConnection = {
        ip:
          primaryAllocation.allocation.alias ||
...
app.post(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      const result = await assignPortToServer(
        serverId,
        allocationId,
        requestedPort ? parseInt(requestedPort) : undefined
      );
...
app.post(
  "/server/:id/allocations/set-primary",
  requireAuth,
  async (req: express.Request, res: express.Response) => {
    try {
      const { allocationId } = req.body;
...
app.post(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      const { name, type, version, memory, cpu, nodeId, allocationId, port } =
        req.body;
      ...
      if (allocationId) {
        ...
        if (allocation && allocation.nodeId === nodeId) {
          ...
          const usedPorts = allocation.servers.map((sa) => sa.port);
          const freePorts = availablePorts.filter(
            (p) => !usedPorts.includes(p)
          );
...
app.get(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${name || path.basename(filePath as string)}"`
      );
      ...
      res.send(
        Buffer.from(
          fileData.content,
          fileData.encoding === "base64" ? "base64" : "utf-8"
        )
      );
    } catch (error: any) {
...
app.post(
  "/admin/nodes/:nodeId/allocations/create",
  requireAdmin,
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      // Validate IP address
      const ipRegex =
        /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$|^0\.0\.0\.0$/;
      ...
      if (!portRegex.test(ports)) {
        return res
          ...
          .json({
            error:
              "Invalid port format. Use format like 25565-25777 or 25565,25566,25567",
          });
      }
...
app.post(
  "/admin/nodes/allocations/:id/update",
  requireAdmin,
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      if (ports) {
        ...
        if (usedPorts.length > 0) {
          ...
          return res.status(400).json({
            error: `Cannot update ports. Ports ${usedPortNumbers.join(
              ", "
            )} are in use by servers`,
          });
        }
      }
...
app.post(
  "/admin/nodes/allocations/:id/delete",
  requireAdmin,
  async (req: express.Request, res: express.Response) => {
    try {
      const allocationId = req.params.id;
...
app.get(
  "/admin/nodes/allocations/:id/ports",
  requireAdmin,
  async (req: express.Request, res: express.Response) => {
    try {
      const allocationId = req.params.id;
      ...
      const allPorts = parsePortRange(allocation.ports);
      const usedPorts = allocation.servers.map((sa) => sa.port);
      const availablePorts = allPorts.filter(
...
app.get(
  "/api/v1/nodes/:nodeId/allocations",
  requireAdmin,
  async (req: express.Request, res: express.Response) => {
    try {
