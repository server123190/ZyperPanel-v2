...
async function getAvailablePorts(allocationId) {
    ...
    if (!allocation)
        return [];
    const allPorts = parsePortRange(allocation.ports);
    const usedPorts = allocation.servers.map((sa) => sa.port);
    return allPorts.filter((port) => !usedPorts.includes(port));
}
async function assignPortToServer(serverId, allocationId, requestedPort) {
    const availablePorts = await getAvailablePorts(allocationId);
    if (availablePorts.length === 0) {
        return { success: false, error: "No available ports in this allocation" };
    }
    let portToUse;
    if (requestedPort) {
        if (availablePorts.includes(requestedPort)) {
            portToUse = requestedPort;
        }
        else {
            return { success: false, error: "Requested port is not available" };
        }
    }
    else {
        portToUse = availablePorts[0];
    }
    // Check if server already has this allocation
    const existingAllocation = await prisma.serverAllocation.findFirst({
        where: {
            serverId,
            allocationId,
        },
    ...
    if (existingAllocation) {
        ...
        await prisma.serverAllocation.update({
        ...
        await prisma.serverAllocation.create({
            data: {
                serverId,
                allocationId,
                port: portToUse,
                isPrimary: true, // Will be adjusted if needed
            },
        });
    }
...
app.post("/account/update", requireAuth, async (req, res) => {
    ...
    if (newPassword) {
        const valid = bcryptjs_1.default.compareSync(oldPassword, user.password);
        if (!valid) {
            return res.send("Old password is incorrect");
        }
        password = bcryptjs_1.default.hashSync(newPassword, 10);
    }
...
// ============== SERVER MANAGEMENT ==============
app.post("/admin/servers/:id/suspend", requireAdmin, async (req, res) => {
...
app.post("/admin/servers/:id/unsuspend", requireAdmin, async (req, res) => {
    await prisma.server.update({
        where: {
            id: req.params.id,
        },
...
app.get("/server/:id", requireAuth, async (req, res) => {
    ...
    // BLOCK SUSPENDED SERVERS
    if (server.status === "suspended") {
        return res.render("user/server-suspended", {
            settings,
            user,
            server,
        });
    }
    // Get primary allocation for connection info
    let primaryConnection = null;
    const primaryAllocation = server.allocations?.find((a) => a.isPrimary);
    if (primaryAllocation) {
        primaryConnection = {
            ip: primaryAllocation.allocation.alias ||
                primaryAllocation.allocation.ipAddress,
...
app.get("/server/:id/allocations", requireAuth, async (req, res) => {
    try {
        const settings = await getSettings();
        const user = await prisma.user.findUnique({
            where: { id: req.session.userId },
        ...
        // Get available allocations for this node
        ...
        res.render("user/server/allocations", {
            settings,
            user,
            server,
            nodeAllocations,
            serverAllocations: server.allocations || [],
            formatBytes: (bytes) => {
                if (bytes === 0)
                    return "0 Bytes";
                const k = 1024;
        ...
        console.error("Allocations route error:", error);
        res.redirect(`/server/${req.params.id}`);
    }
});
app.post("/server/:id/allocations/add", requireAuth, async (req, res) => {
    try {
        const { allocationId, port: requestedPort } = req.body;
        ...
        const result = await assignPortToServer(serverId, allocationId, requestedPort ? parseInt(requestedPort) : undefined);
        if (!result.success) {
            return res.status(400).json({ error: result.error });
        }
        // If this is the first allocation, set it as primary
        const serverAllocations = await prisma.serverAllocation.count({
            where: { serverId },
        });
        if (serverAllocations === 1) {
...
app.post("/server/:id/allocations/remove", requireAuth, async (req, res) => {
    try {
        ...
        const allocation = await prisma.serverAllocation.findFirst({
            where: {
                serverId,
                allocationId,
            },
        ...
        await prisma.serverAllocation.delete({
            where: { id: allocation.id },
        });
        // If we removed the primary allocation, set another as primary
        if (allocation.isPrimary) {
            const remainingAllocations = await prisma.serverAllocation.findFirst({
                where: { serverId },
            });
            if (remainingAllocations) {
                await prisma.serverAllocation.update({
                    where: { id: remainingAllocations.id },
                    data: { isPrimary: true },
                });
            }
        }
...
app.post("/server/:id/allocations/set-primary", requireAuth, async (req, res) => {
    try {
        const { allocationId } = req.body;
        const serverId = req.params.id;
        const server = await prisma.server.findUnique({
        ...
        res.json({ success: true, message: "Primary allocation updated" });
    }
    catch (error) {
        console.error("Set primary allocation error:", error);
        res.status(500).json({ error: error.message });
    }
});
...
app.post("/server/create", requireAuth, async (req, res) => {
    try {
        const { name, type, version, memory, cpu, nodeId, allocationId, port } = req.body;
        const node = await prisma.node.findUnique({ where: { id: nodeId } });
        if (!node) {
        ...
        // If allocation is specified, assign a port from it
        if (allocationId) {
            const allocation = await prisma.nodeAllocation.findUnique({
                where: { id: allocationId },
                include: { servers: true },
            ...
            if (allocation && allocation.nodeId === nodeId) {
                const availablePorts = parsePortRange(allocation.ports);
                const usedPorts = allocation.servers.map((sa) => sa.port);
                const freePorts = availablePorts.filter((p) => !usedPorts.includes(p));
            ...
            // Use auto-generated port
            serverPort = 25565 + userServers;
        }
        const containerData = {
            name: containerName,
        ...
        // If allocation was specified, create the allocation record
        if (allocationId) {
            await prisma.serverAllocation.create({
                data: {
                    serverId: server.id,
...
app.post("/server/:id/files/rename", requireAuth, async (req, res) => {
    try {
        ...
        const result = await NodeAPI.call(server.node.id, `/instances/${server.containerId}/files/rename`, "POST", { oldPath, newName, isDir });
        res.json(result);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
// Download file
app.get("/server/:id/files/download", requireAuth, async (req, res) => {
    try {
        ...
        const fileData = await NodeAPI.call(server.node.id, `/instances/${server.containerId}/files/read`, "POST", { filePath });
        if (!fileData.success) {
            return res.status(404).send("File not found");
        }
        // Set download headers
        res.setHeader("Content-Disposition", `attachment; filename="${name || path_1.default.basename(filePath)}"`);
...
app.get("/admin/nodes/:nodeId/allocations", requireAdmin, async (req, res) => {
    try {
        const settings = await getSettings();
        ...
        if (!node) {
            return res.redirect("/admin/nodes?error=Node not found");
        }
        res.render("admin/nodes/allocations", {
            settings,
            user: req.user,
            node,
        ...
        console.error("Allocations route error:", error);
        res.redirect("/admin/nodes?error=" + encodeURIComponent(error.message));
    }
});
app.post("/admin/nodes/:nodeId/allocations/create", requireAdmin, async (req, res) => {
    try {
        const { ipAddress, alias, ports, isPrimary } = req.body;
        const nodeId = req.params.nodeId;
        // Validate IP address
        const ipRegex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$|^0\.0\.0\.0$/;
        ...
        if (!portRegex.test(ports)) {
            return res
                .status(400)
                .json({
                error: "Invalid port format. Use format like 25565-25777 or 25565,25566,25567",
            });
        }
        // Parse ports to check they're valid
        const parsedPorts = parsePortRange(ports);
        if (parsedPorts.length === 0) {
            return res.status(400).json({ error: "No valid ports found in range" });
        }
        // If setting as primary, unset other primaries
        if (isPrimary === "yes" || isPrimary === true) {
            await prisma.nodeAllocation.updateMany({
                where: { nodeId, isPrimary: true },
                data: { isPrimary: false },
            });
        }
        const allocation = await prisma.nodeAllocation.create({
            data: {
                nodeId,
                ipAddress,
                alias: alias || null,
                ports,
                isPrimary: isPrimary === "yes" || isPrimary === true,
            },
        });
        res.json({
            success: true,
...
app.post("/admin/nodes/allocations/:id/update", requireAdmin, async (req, res) => {
    try {
        const { ipAddress, alias, ports, isActive, isPrimary } = req.body;
        ...
        if (ports) {
            const portRegex = /^(\d+(-\d+)?)(,\s*\d+(-\d+)?)*$/;
            if (!portRegex.test(ports)) {
                return res.status(400).json({ error: "Invalid port format" });
            }
            const parsedPorts = parsePortRange(ports);
            if (parsedPorts.length === 0) {
                return res.status(400).json({ error: "No valid ports found" });
            }
            // Check if any of the ports are in use (excluding this allocation)
            const usedPorts = await prisma.serverAllocation.findMany({
                where: {
                    allocationId,
                    port: {
                        in: parsedPorts,
            ...
            if (usedPorts.length > 0) {
                const usedPortNumbers = usedPorts.map((up) => up.port);
                return res.status(400).json({
                    error: `Cannot update ports. Ports ${usedPortNumbers.join(", ")} are in use by servers`,
                });
            }
        }
        // If setting as primary, unset other primaries
        ...
        const updatedAllocation = await prisma.nodeAllocation.update({
            where: { id: allocationId },
            data: {
                ipAddress: ipAddress || allocation.ipAddress,
                alias: alias !== undefined ? alias || null : allocation.alias,
                ports: ports || allocation.ports,
                isActive: isActive !== undefined
                    ? isActive === "yes" || isActive === true
                    : allocation.isActive,
                isPrimary: isPrimary !== undefined
                    ? isPrimary === "yes" || isPrimary === true
...
app.post("/admin/nodes/allocations/:id/delete", requireAdmin, async (req, res) => {
    try {
        const allocationId = req.params.id;
        const allocation = await prisma.nodeAllocation.findUnique({
            where: { id: allocationId },
        ...
        // Check if allocation has servers using it
        if (allocation.servers.length > 0) {
            return res.status(400).json({
                error: "Cannot delete allocation. It is currently in use by servers.",
            });
        }
        await prisma.nodeAllocation.delete({
            where: { id: allocationId },
        });
        // If this was primary, set another as primary
        if (allocation.isPrimary) {
            const nextPrimary = await prisma.nodeAllocation.findFirst({
                where: {
                    nodeId: allocation.nodeId,
                    id: { not: allocationId },
                },
...
app.get("/admin/nodes/allocations/:id/ports", requireAdmin, async (req, res) => {
    try {
        const allocationId = req.params.id;
        const allocation = await prisma.nodeAllocation.findUnique({
            where: { id: allocationId },
        ...
        const allPorts = parsePortRange(allocation.ports);
        const usedPorts = allocation.servers.map((sa) => sa.port);
        const availablePorts = allPorts.filter((port) => !usedPorts.includes(port));
        res.json({
            success: true,
            allocation: {
                id: allocation.id,
            ...
            ports: {
                ...
                usedDetails: allocation.servers.map((sa) => ({
                    port: sa.port,
                    server: sa.server,
                })),
            },
...
app.get("/admin/overview", requireAdmin, async (req, res) => {
    ...
    const allocations = await prisma.nodeAllocation.count();
    ...
    res.render("admin/overview/overview", {
        settings,
        user: req.user,
        stats: { users, servers, nodes, allocations },
        recentLogs,
    });
});
...
app.post("/admin/servers/create", requireAdmin, async (req, res) => {
    const { name, type, version, port, memory, cpu, nodeId, userId, allocationId, } = req.body;
    try {
        const node = await prisma.node.findUnique({ where: { id: nodeId } });
        if (!node)
            throw new Error("Node not found");
        const containerName = `mc-${name
            .toLowerCase()
            .replace(/[^a-z0-9]/g, "-")}-${Date.now()}`;
        let serverPort = parseInt(port) || 25565;
        // If allocation is specified, assign a port from it
        if (allocationId && allocationId !== "none") {
            const allocation = await prisma.nodeAllocation.findUnique({
                where: { id: allocationId },
                include: { servers: true },
            });
            if (allocation && allocation.nodeId === nodeId) {
                ...
                if (freePorts.length > 0) {
                    ...
                    throw new Error("No available ports in selected allocation");
                }
            }
        }
        const containerData = {
...
app.get("/api/v1/nodes/:nodeId/allocations", requireAdmin, async (req, res) => {
    try {
