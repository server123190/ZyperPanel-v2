...
app.post("/instances/:id/files/rename", authMiddleware, async (req, res) => {
  try {
    ...
    const oldFullPath = path.join(server.directory, oldPath);
    ...
    if (
      !oldResolved.startsWith(serverDir) ||
      !newResolved.startsWith(serverDir)
    ) {
    ...
    if (fs.existsSync(newFullPath)) {
      return res
        .status(400)
        .json({ error: "A file or directory with that name already exists" });
    }
    ...
    res.json({
      success: true,
      message: `${isDir ? "Directory" : "File"} renamed successfully`,
      oldPath: oldPath,
      newPath: path.relative(server.directory, newFullPath),
    });
  } catch (error) {
...
app.post("/instances/:id/files/delete", authMiddleware, async (req, res) => {
  try {
    ...
    const { filePath: rawFilePath, force = false } = req.body;
    const serverId = req.params.id;
    const server = config.servers[serverId];
    ...
    // ✅ Decode the path (e.g. "plugins%2FViaVersion.jar" -> "plugins/ViaVersion.jar")
    ...
    if (
      !requestedPath.startsWith(serverDir + path.sep) &&
      requestedPath !== serverDir
    ) {
      return res.status(403).json({ error: "Access denied" });
    }
